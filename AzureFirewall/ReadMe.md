...
```New-AzSubscriptionDeployment -Name demoSubDeployment -Location centralus -TemplateUri "https://raw.githubusercontent.com/Azure/azure-docs-json-samples/master/azure-resource-manager/emptyrg.json" -AdminPassword "@$tr0NGp@sswOrb22" -SharedKey "$3cuR3MyvpN!78" ```  
...
We will walk through the steps to build out the environment to configure routing required to force tunnel traffic through the Azure Firewall. At this point, you should have 2 Resource Groups, 3 Virtual Networks, and a single Azure Firewall with Firewall Policy. We’ll be creating 1 more Azure Firewall to represent the on-premises downstream firewall, 2 Virtual Network Gateways to create our IKEv2 w/BGP tunnel, and 4 Route Tables to properly route packets to their respective destination.
...
For the on-premises firewall, we’ll use the same steps from Section 1 with defined changes. 
